...
require('./settings')
const { default: makeWASocket, useMultiFileAuthState, DisconnectReason, MessageRetryMap, fetchLatestBaileysVersion, AnyMessageContent, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
ppuser = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png'
  ...
  try {
    if (anu.action == 'add' && num.includes(ichi.user.id)) {
      ichi.sendMessage(gruplog, { text: `ID Grup ${metadata.subject} adalah: \n${anu.id}\n\nJangan lupa kirim ke Owner BOT ya:)` })
    }
  } catch {
    if (anu.action == 'add' && num.includes(ichi.user.id)) {
      ichi.sendMessage(gruplog, { text: `ID Grup ${metadata.subject} adalah: \n${anu.id}\n\nJangan lupa kirim ke Owner BOT ya:)` })
...
if (anu.action == 'add') {
tekswell = `*Selamat datang @${num.split('@')[0]} di Grup ${metadata.subject} 👋*\n\nJangan lupa baca rules grup\nKetik Help untuk menggunakan BOT`
     ...
     ichi.ev.on("call", async (senku) => {
        for (let sen of senku) {
         ...
         teks = `Kamu terdeteksi menelepon BOT, Kamu akan di *Block dan masuk daftar Banned*, jika tidak sengaja silahkan gabung grup untuk melepas *Block dan Banned* kamu \n\nLink Grup: https://chat.whatsapp.com/K3rILprlI0xHS1IxYQqwJo`
          ...
          await ichi.sendMessage(gruplog, { text: `Ada yang telepon bot dengan nomer wa.me/${sen.chatId.split("@")[0]}`})
            }
        } return
     })
...
ichi.ev.on("messages.reaction", async (pesan) => {
	try {
	  var m = pesan[0]
		...
		let frem = m.reaction.key.remoteJid ? m.reaction.key.participant : m.reaction.key.remoteJid;
